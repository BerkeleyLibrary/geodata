...
describe EndpointUrl do
  ...
  describe '.get_url' do
    ...
    context 'when file exists with valid URL' do
      before do
        allow(Rails.configuration.x.servers).to receive(:[]).with('test_server').and_return(secret_file)
        File.write(secret_file, 'https://user:pass@example.com/rest/')
      end
      ...
      it 'returns cleaned URL without credentials and /rest suffix' do
        result = EndpointUrl.get_url('test_server')
    ...
    context 'when server config is nil' do
      before do
        allow(Rails.configuration.x.servers).to receive(:[]).with('test_server').and_return(nil)
      end
  ...
  describe '.safe_url' do
    ...
    it 'removes both credentials and /rest suffix' do
      result = EndpointUrl.safe_url('https://user:password@example.com/rest/')
      expect(result).to eq('https://example.com')
    end

    it 'returns nil when input is nil' do
      result = EndpointUrl.safe_url(nil)
      expect(result).to be_nil
    end
  end
end
